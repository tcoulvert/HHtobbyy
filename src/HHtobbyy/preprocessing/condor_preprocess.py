...
class LPCVanillaSubmitter:
    ...
    def __init__(
        ...
        # Not sumitting duplicate jobs unless forcing
        if not self.force:
            for era in era_filepaths.keys():
                era_filepaths[era] = [
        ...
        for era, filepaths in era_filepaths.items():
            ...
            with open(job_file_executable, "w") as executable_file:
                ...
                executable_file.write("python3.12 -m venv /srv/bbyy_venv\n")
                executable_file.write("source /srv/bbyy_venv/bin/activate\n")
                ...
                executable_file.write(f"echo \"Running pre-processing for {era}\"\n")
                executable_file.write("echo \"-------------------------------------\"\n")
    ...
    def update_git(self):
        ...
        if comm_code != 0 and not 'branch is up to date' in comm_err: raise Exception('Git commit failed')
        push_code, push_out, push_err = run_git_cmd(["git", "push"])
        if push_code != 0: raise Exception('Git push failed')
