...
class MLPDataset(ModelDataset):
    ...
    def get_MLPTorch(self, df: pd.DataFrame, event_weight: str):
        return MLPTorchDataset(df[self.dfdataset.model_vars].to_numpy(), df[f"{self.dfdataset.aux_var_prefix}label1D"].to_numpy(), np.abs(df[f"{self.dfdataset.aux_var_prefix}{event_weight}"].to_numpy()))
